// https://unpkg.com/bss@1.6.4/bss.esm.js?module
var pseudos = [
    ":active",
    ":any",
    ":checked",
...
var cssProperties = ["float"].concat(
    Object.keys(
...
function add(style, prop, values) {
    if (prop in style) {
        // Recursively increase specificity
...
function hyphenToCamelCase(hyphen) {
    return hyphen
        .slice(hyphen.charAt(0) === "-" ? 1 : 0)
        .replace(hyphenSeparator, function (match) {
...
function initials(camelCase) {
    return (
        camelCase.charAt(0) +
        (camelCase.match(initialMatch) || []).join("").toLowerCase()
    );
}
...
function objectToRules(style, selector, suffix, single) {
    ...
    Object.keys(style).forEach(function (prop) {
        if (prop.charAt(0) === "@") {
            rules.push(
                prop +
                    "{" +
                    objectToRules(style[prop], selector, suffix, single).join(
                        ""
                    ) +
                    "}"
            );
            ...
            rules = rules.concat(
                objectToRules(style[prop], selector, suffix + prop, single)
            );
        } else {
    ...
    if (Object.keys(base).length) {
        rules.unshift(
            ((single || suffix.charAt(0) === " " ? "" : "&") + extra + suffix)
                .replace(ampersandMatch, selector)
                .trim() +
                "{" +
                stylesToCss(base) +
                "}"
...
function stylesToCss(style) {
    return Object.keys(style).reduce(function (acc, prop) {
        return (
            acc +
            propToString(
                prop.charAt(0) === "!" ? prop.slice(1) : prop,
                style[prop]
            )
        );
    }, "");
...
function propToString(prop, value) {
    ...
    return (
        (vendorRegex.test(prop) ? "-" : "") +
        (cssVar(prop) ? prop : camelCaseToHyphen(prop)) +
        ":" +
        value +
        ";"
    );
...
var styleSheet =
    typeof document === "object" && document.createElement("style");
...
function insert(rule, index) {
    ...
    try {
        sheet &&
            sheet.insertRule(
                rule,
                arguments.length > 1 ? index : sheet.cssRules.length
            );
    } catch (e) {
        ...
        style: {
            ...
            get: function () {
                ...
                return Object.keys(this.__style).reduce(function (acc, key) {
                    if (
                        typeof this$1.__style[key] === "number" ||
                        typeof this$1.__style[key] === "string"
                    ) {
                        acc[key.charAt(0) === "!" ? key.slice(1) : key] =
                            this$1.__style[key];
                    }
                    return acc;
                }, {});
    ...
    if (vendor) {
        ...
        if (cssProperties.indexOf(unprefixed) === -1) {
            if (unprefixed === "flexDirection") {
                vendorValuePrefix.flex =
                    "-" + vendor[1].toLowerCase() + "-flex";
            }
...
function $import(value) {
    value &&
        insert(
            "@import " + (hasUrl.test(value) ? value : '"' + value + '"') + ";",
            0
        );
...
pseudos.forEach(function (name) {
    return setProp(
        "$" + hyphenToCamelCase(name.replace(/:/g, "")),
        function Pseudo(value, style) {
            var b = chain(this);
            if (isTagged(value)) {
                b.__style[name] = parse.apply(null, arguments);
            } else if (value || style) {
                b.__style[name + (style ? "(" + value + ")" : "")] = parse(
                    style || value
                );
            }
            return b;
