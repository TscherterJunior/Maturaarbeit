    ...
    if (input.length) {
        ...
        for (var i = 1; i < input.length; i++) {
            if ((input[i] != null && input[i].key != null) !== isKeyed) {
                throw new TypeError(
                    isKeyed &&
                    (input[i] != null || typeof input[i] === "boolean")
    ...
    function updateEvent(vnode3, key, value) {
        if (vnode3.events != null) {
            ...
            if (
                ...
                if (vnode3.events[key] != null)
                    vnode3.dom.removeEventListener(
                        key.slice(2),
                        vnode3.events,
                        false
                    );
    ...
    function shouldNotUpdate(vnode3, old) {
        do {
            if (
                ...
                var force = callHook.call(
                    vnode3.attrs.onbeforeupdate,
                    vnode3,
                    old
                );
...
var _19 = function ($window, Promise, oncompletion) {
    ...
    return {
        request: makeRequest(function (url, args, resolve, reject) {
            var method =
                args.method != null ? args.method.toUpperCase() : "GET";
            var body = args.body;
            var assumeJSON =
...
var parseQueryString = function (string) {
    ...
    for (var i = 0; i < entries.length; i++) {
        ...
        var value2 =
            entry.length === 2 ? decodeURIComponentSave0(entry[1]) : "";
                ...
                if (compiled[i].check(data)) {
                    ...
                    if (payload.view || typeof payload === "function") {
                        ...
                        p.then(function () {
                            ...
                            update,
                            path0 === fallbackRoute ? null : reject
                        );
                    } else update("div");
                    return;
                }
...
//m    if (typeof module !== "undefined") module["exports"] = m
//m    else window.m = m
//m    }());
export default m;
